...
#RequestHandler :从URL函数中分析其需要接收的参数，从request中获取必要的参数，调用URL函数，
...
#自动把handler模块的所有符合条件的函数都注册上
def add_routes(app,module_name):
	n = module_name.rfind('.')
	if n == (-1): # -1即代表没有找到匹配的字符串 @wuyan 20180201
		mod = __import__(module_name,globals(),locals())
	else:
		name = module_name[n+1:] #fixme n+1:是什么意思？
		mod = getattr(__import__(module_name[:n],globals(),locals(),[name]),name) #fixme :n 是什么意思？
	for attr in dir(mod):
