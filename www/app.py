...
async def data_factory(app,handler):
	async def parse_data(request):
		if request.method == 'POST':
			if request.content-type.startswith('application/json'):
				request.__data__ = await request.json()
				logging.info('request json: %s' % str(request.__data__))
			elif request.content-type.startswith('application/x-www-form-urlencoded'):
...
#@asyncio.coroutine
async def init(loop):
	await orm.create_pool(loop=loop, host='127.0.0.1', post=3306, user='root', password='123456', db='wuyan')
	app = web.Application(loop=loop,middlewares=[logger_factory, response_factory])
	init_jinja2(app,filters=dict(datetime=date_time_filter))
